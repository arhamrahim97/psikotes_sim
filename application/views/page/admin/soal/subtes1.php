		...
		<div class="col-xl-12 col-lg-12">
			<div class="card shadow mb-4">
				...
				<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
					<h6 class="m-0 font-weight-bold text-primary">Soal Stabilitas Emos</h6>
							...
							<div class="modal-dialog modal-xl" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">Tambah Soal Subtes 1</h5>
					...
					<div class="modal fade" id="modalEditSubtes" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
						<form method="POST">
							<div class="modal-dialog modal-xl" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">Edit Soal Subtes 1</h5>
										<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span aria-hidden="true">&times;</span>
										</button>
									</div>
									<div class="modal-body">
										...
										<div class="form-group">
											<label for="namaKecamatan">Nilai Jawaban A</label>
											<input type="number" class="form-control" id="nilai_jawaban_a_edit" placeholder="Masukkan Nilai">
										</div>
										<div class="form-group">
											<label for="namaKecamatan">Jawaban B</label>
										...
										<div class="form-group">
											<label for="namaKecamatan">Nilai Jawaban B</label>
											<input type="number" class="form-control" id="nilai_jawaban_b_edit" placeholder="Masukkan Nilai">
										</div>
										<div class="form-group">
											<label for="namaKecamatan">Jawaban C</label>
										...
										<div class="form-group">
											<label for="namaKecamatan">Nilai Jawaban C</label>
											<input type="number" class="form-control" id="nilai_jawaban_c_edit" placeholder="Masukkan Nilai">
										</div>
										<div class="form-group">
											<label for="exampleFormControlSelect1">Tipe</label>
...
<script>
	...
	$(document).ready(function() {
		$("#tabel").DataTable({
			ajax: {
				url: '<?php echo base_url('getSubtes1') ?>',
				dataSrc: ''
			},
			scrollX: true,
	...
	$("#btnUpdate").click(function() {
		...
		if (no_soal_edit == '') {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url('editSubtes1') ?>',
				data: {
				...
				success: function(data) {
					if (data == 'true') {
						...
						Swal.fire({
							icon: 'error',
							title: 'Terjadi Kesalahan',
							text: 'No Soal Subtes 1 dan Tipe Yang Anda Masukkan Sudah Ada',
						})
					}
				},
				error: function(data) {
	...
	$("#btnSimpan").click(function() {
		...
		if (no_soal == '') {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url('insertSubtes1') ?>',
				data: {
					no_soal: no_soal,
				...
				success: function(data) {
					if (data == 'true') {
						Swal.fire({
							icon: 'success',
							title: 'Berhasil',
							text: 'Soal Subtes 1 Berhasil Disimpan',
						})
						refreshTable();
						$("#no_soal").val('');
	...
	function edit(id) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('getEditSubtes1') ?>',
			data: {
				id: id
			},
			dataType: "JSON",
	...
	function detail(id) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('getDetailSubtes1') ?>',
			data: {
				id: id
			},
			success: function(data) {
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url('hapusSubtes1') ?>',
					data: {
						id: id
					},
					success: function(data) {
