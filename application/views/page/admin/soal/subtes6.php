		...
		<div class="col-xl-12 col-lg-12">
			<div class="card shadow mb-4">
				...
				<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
					<h6 class="m-0 font-weight-bold text-primary">Soal Konsentrasi</h6>
							...
							<div class="modal-dialog modal-xl" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">Tambah Soal Subtes 6</h5>
									...
									<div class="modal-body">
										...
										<div class="form-group">
											<label for="namaKecamatan">Jawaban D</label>
											<textarea name="editor9" id="editor9" class="jawaban_d">
                                            </textarea>
										</div>
										<div class="form-group">
											<label for="namaKecamatan">Nilai Jawaban D</label>
											<input type="number" class="form-control" id="nilai_jawaban_d" placeholder="Masukkan Nilai">
										</div>
										<div class="form-group">
					...
					<div class="modal fade" id="modalEditSubtes" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
						<form method="POST">
							<div class="modal-dialog modal-xl" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">Edit Soal Subtes 6</h5>
										<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span aria-hidden="true">&times;</span>
										</button>
									</div>
									<div class="modal-body">
										...
										<div class="form-group">
											<label for="namaKecamatan">Jawaban D</label>
											<textarea name="editor10" id="editor10" class="jawaban_d_edit">
                                            </textarea>
										</div>
										<div class="form-group">
											<label for="namaKecamatan">Nilai Jawaban D</label>
											<input type="number" class="form-control" id="nilai_jawaban_d_edit" placeholder="Nilai">
										</div>
										<div class="form-group">
											<label for="exampleFormControlSelect1">Tipe</label>
...
<script>
	...
	$(document).ready(function() {
		$("#tabel").DataTable({
			ajax: {
				url: '<?php echo base_url('getSubtes6') ?>',
				dataSrc: ''
			},
			scrollX: true,
	...
	$("#btnUpdate").click(function() {
		...
		var jawaban_d_edit = CKEDITOR.instances['editor10'].getData();
		var nilai_jawaban_d_edit = $("#nilai_jawaban_d_edit").val();
		...
		} else if (jawaban_d_edit == "") {
			Swal.fire({
				icon: 'error',
				title: 'Terjadi Kesalahan',
				text: 'Jawaban D Tidak Boleh Dikosongkan',
			})
		} else if (nilai_jawaban_d_edit == "") {
			Swal.fire({
				icon: 'error',
				title: 'Terjadi Kesalahan',
				text: 'Nilai Jawaban D Tidak Boleh Dikosongkan',
			})
		} else {
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url('editSubtes6') ?>',
				data: {
					...
					jawaban_d_edit: jawaban_d_edit,
					nilai_jawaban_d_edit: nilai_jawaban_d_edit,
					tipe_edit: tipe_edit
				},
				success: function(data) {
					if (data == 'true') {
						...
						Swal.fire({
							icon: 'error',
							title: 'Terjadi Kesalahan',
							text: 'No Soal Subtes 5 dan Tipe Yang Anda Masukkan Sudah Ada',
						})
					}
				},
				error: function(data) {
	...
	$("#btnSimpan").click(function() {
		...
		var jawaban_d = CKEDITOR.instances['editor9'].getData();
		var nilai_jawaban_d = $("#nilai_jawaban_d").val();
		...
		if (no_soal == '') {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url('insertSubtes6') ?>',
				data: {
					no_soal: no_soal,
					...
					jawaban_d: jawaban_d,
					nilai_jawaban_d: nilai_jawaban_d,
					tipe: tipe
				},
				success: function(data) {
					if (data == 'true') {
						Swal.fire({
							icon: 'success',
							title: 'Berhasil',
							text: 'Soal Subtes 6 Berhasil Disimpan',
						})
						refreshTable();
						$("#no_soal").val('');
						...
						Swal.fire({
							icon: 'error',
							title: 'Terjadi Kesalahan',
							text: 'No Soal Subtes 6 dan Tipe Yang Anda Masukkan Sudah Ada',
						})
					}
				},
				error: function(data) {
	...
	function edit(id) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('getEditSubtes6') ?>',
			data: {
				id: id
			},
			dataType: "JSON",
	...
	function detail(id) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('getDetailSubtes6') ?>',
			data: {
				id: id
			},
			success: function(data) {
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url('hapusSubtes6') ?>',
					data: {
						id: id
					},
					success: function(data) {
...
<script>
	...
	CKEDITOR.replace('editor9', { // attribute id komponen textarea
		height: '100px',
		filebrowserImageBrowseUrl: '<?php echo base_url('assets/vendor/kcfinder/browse.php'); ?>'
	});

	CKEDITOR.replace('editor10', { // attribute id komponen textarea
		height: '100px',
		filebrowserImageBrowseUrl: '<?php echo base_url('assets/vendor/kcfinder/browse.php'); ?>'
	});
</script>
